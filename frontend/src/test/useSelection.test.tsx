...
describe('useSelection', () => {
  it('initial state has empty selections and all can* false', () => {
    const { result } = renderHook(() =>
      useSelection({
        filteredEntries: emptyEntries,
        trashEntries: emptyTrash,
        favorites: [],
        canWrite: true,
        currentPath: '/root',
      }),
  ...
  it('selecting one file sets canRename, canDownload, canPreview, canArchive to true', () => {
    ...
    act(() => {
      result.current.handleContextMenuEvent(
        entries[0]!,
        {
  ...
  it('shift-click on trash selects a range', () => {
    ...
    act(() => {
      result.current.handleSelectTrashItem(trash[0]!, {
        shiftKey: false,
        metaKey: false,
        ctrlKey: false,
    ...
    act(() => {
      result.current.handleSelectTrashItem(trash[2]!, {
        shiftKey: true,
        metaKey: false,
        ctrlKey: false,
      } as React.MouseEvent<HTMLElement>);
  ...
  it('cmd-click on trash toggles selection', () => {
    ...
    act(() => {
      result.current.handleSelectTrashItem(trash[1]!, {
        shiftKey: false,
        metaKey: true,
        ctrlKey: false,
      } as React.MouseEvent<HTMLElement>);
  ...
  it('context menu selects a new entry when not already selected', () => {
    ...
    act(() => {
      const outcome = result.current.handleContextMenuEvent(
        entries[1]!,
        {
          preventDefault: vi.fn(),
          stopPropagation: vi.fn(),
