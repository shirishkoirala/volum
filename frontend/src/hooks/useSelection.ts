...
export function useSelection({ filteredEntries, trashEntries, favorites, canWrite, currentPath }: UseSelectionParams) {
  ...
  const handleContextMenuEvent = useCallback((entry: FileEntry, event: MouseEvent<HTMLElement>, renaming: boolean): { blocked: boolean; shouldSelect: boolean } => {
  ...
  return {
    ...
    lastSelectedPath, setLastSelectedPath,
    selectedTrashIds, setSelectedTrashIds,
    lastSelectedTrashId, setLastSelectedTrashId,
    selectedEntries,
    isFavorited,
    canRename, canDownload, canDelete, canCopy, canMove, canInfo, canPreview, canArchive, canExtract, canChecksum, canAnalyze, canPaste,
    handleSelectAll, handleInvertSelection,
    handleFileClick, handleWorkspaceClick,
