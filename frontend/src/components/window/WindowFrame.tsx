...
type SnapTarget = 'maximize' | 'left' | 'right' | 'topLeft' | 'topRight' | 'bottomLeft' | 'bottomRight';
...
function getSnapRect(target: SnapTarget): WindowRect {
  ...
  switch (target) {
    ...
    case 'bottomRight':
      return { x: halfW, y: area.y + halfH, width: area.width - halfW, height: area.height - halfH };
  }
}
...
export function WindowFrame({ win, children }: { win: WindowState; children?: React.ReactNode }) {
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    const handleMouseMove = (ev: MouseEvent) => {
      ...
      const newX = Math.max(-win.width + MIN_EDGE_DISTANCE, Math.min(window.innerWidth - MIN_EDGE_DISTANCE, startLeft + dx));
  ...
  }, [focusWindow, win.id, win.x, win.y, win.width, win.height, isMobile, isMaximized, updatePosition, updateSize, toggleMaximize]);
  ...
  const handleResizeStart = useCallback((dir: ResizeDir, e: React.MouseEvent) => {
    ...
    const handleMouseMove = (ev: MouseEvent) => {
      ...
      if (dir.includes('w')) { w = Math.max(MIN_WINDOW_W, startW - dx); x = startL + startW - w; }
      if (dir.includes('s')) h = Math.max(MIN_WINDOW_H, startH + dy);
      if (dir.includes('n')) { h = Math.max(MIN_WINDOW_H, startH - dy); y = startT + startH - h; }
